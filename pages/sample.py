...
for category, items in foods.items():
    ...
    for item, portion in items:
        ...
        total_intake = freq_map[frequency] * amount #ここで摂取総量を求めている
...
if st.button("送信"):
    ...
    nutrients_dict = nutrients_csv.set_index("食品")["エネルギー(kcal)"].to_dict()
    df["総エネルギー(kcal)"] = df["摂取総量"] * df["食品"].map(nutrients_dict)
